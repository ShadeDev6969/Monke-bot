...
module.exports = {
  ...
  run: async ( message, args) => {
    ...
    const questions = [
      ...
      "Tell me a link or upload a attachment for the author image",
      "Tell me the author link",
      "Tell me the title",
      "Tell me the embed link",
      "Tell me a description",
      "Tell me a thumbnail link or upload a attachment",
      "Tell me a image link or upload a attachment",
      "Tell me a footer text",
      "Tell me a footer image link or upload a attachment",
      "Tell me the color to put it on the embed",
      "Do you want fields?",
    ];
          ...
          .catch((reason) => {
            if (reason === "idle") {
              message.channel.send(
                "Your time is over (2 minutes). Run this command again if you want a embed"
              );
            } else {
              message.channel
                .send("Collector ended with reason: " + reason)
                .catch(() => {});
            }
          });
...
function fields(message, embed) {
  return new Promise((resolve, reject) => {
    ...
    let des = "";
    const arr = [
      "Tell me the field name",
      "Tell me the field value",
      "Want this to be a inline field?",
      "Want another field?",
    ];
